...
export const IMarkSelectionService = createIdentifier<IMarkSelectionService>(
    'pagepeek.mark-selection-service' // NOTE: change to pagepeek.awareness-mark-selection-service
);
 ...
 */
export class MarkSelectionService
    extends Disposable
    implements IMarkSelectionService {
    private _shapeMap: Map<string, IMarkSelectionInfo> = new Map();

    constructor(
        @IUniverInstanceService
        private readonly _currentService: IUniverInstanceService,
    ...
    addShape(
        ...
        const workbook = this._currentService.getCurrentUnitForType<Workbook>(
    ...
    refreshShapes(): void {
        const currentSheet = this._currentService.getCurrentUnitForType<Workbook>(
            UniverInstanceType.UNIVER_SHEET
        );
