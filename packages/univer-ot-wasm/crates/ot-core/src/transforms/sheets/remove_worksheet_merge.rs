...
use crate::mutations::sheets::{
    RemoveWorksheetMergeMutation, InsertRowMutation, InsertColMutation, RemoveRowMutation,
    ...
    InsertSheetMutation, SetWorkbookNameMutation, AddWorksheetMergeMutation, SetColDataMutation,
    RemoveRangeThemeMutation, RemoveSheetMutation, SetWorksheetNameMutation, SetWorksheetOrderMutation,
    SetWorksheetHideMutation, CopyWorksheetEndMutation, AddRangeProtectionMutation,
    DeleteRangeProtectionMutation, AddRangeThemeMutation,
    RemoveWorksheetMergeMutationParams, InsertRowMutationParams, InsertColMutationParams,
...
pub fn register_transforms(registry: &mut TransformRegistry) {
    // Self-transform (RemoveWorksheetMerge vs RemoveWorksheetMerge)
    ...
    registry.register_bidirectional_ref(
        ...
        MUTATION_ID,
        create_insert_row_vs_remove_merge(),
    );
    ...
    registry.register_bidirectional_ref(
        ...
        MUTATION_ID,
        create_insert_col_vs_remove_merge(),
    );
    ...
    registry.register_bidirectional_ref(
        ...
        MUTATION_ID,
        create_remove_row_vs_remove_merge(),
    );
    ...
    registry.register_bidirectional_ref(
        ...
        MUTATION_ID,
        create_remove_col_vs_remove_merge(),
    );
    ...
    registry.register_identity(MUTATION_ID, SetColDataMutation::ID);
    ...
    // Identity with additional mutations
    registry.register_identity(MUTATION_ID, AddRangeProtectionMutation::ID);
    registry.register_identity(MUTATION_ID, DeleteRangeProtectionMutation::ID);
    registry.register_identity(MUTATION_ID, AddRangeThemeMutation::ID);
    registry.register_identity(MUTATION_ID, RemoveRangeThemeMutation::ID);
    registry.register_identity(MUTATION_ID, RemoveSheetMutation::ID);
    registry.register_identity(MUTATION_ID, SetWorksheetNameMutation::ID);
    registry.register_identity(MUTATION_ID, SetWorksheetOrderMutation::ID);
    registry.register_identity(MUTATION_ID, SetWorksheetHideMutation::ID);
    registry.register_identity(MUTATION_ID, CopyWorksheetEndMutation::ID);
}
...
/// InsertRow vs RemoveWorksheetMerge: shift merge range rows >= insert position
fn create_insert_row_vs_remove_merge() -> TransformFnRef {
    Arc::new(|m1: &MutationInfo, m2: &MutationInfo| {
        ...
        // Parse RemoveWorksheetMerge params (m2)
        let mut m2_params: RemoveWorksheetMergeMutationParams = match serde_json::from_value(m2.params.clone()) {
            Ok(p) => p,
            Err(_) => return parse_error(m1, m2, "Failed to parse m2 (RemoveWorksheetMerge) params"),
        };
...
/// InsertCol vs RemoveWorksheetMerge: shift merge range columns >= insert position
fn create_insert_col_vs_remove_merge() -> TransformFnRef {
    Arc::new(|m1: &MutationInfo, m2: &MutationInfo| {
        // Quick worksheet check BEFORE parsing
        if let Some(false) = same_worksheet(&m1.params, &m2.params) {
            return identity(m1, m2);
...
/// RemoveRow vs RemoveWorksheetMerge: shift/remove merge ranges based on removed rows
fn create_remove_row_vs_remove_merge() -> TransformFnRef {
    Arc::new(|m1: &MutationInfo, m2: &MutationInfo| {
        // Quick worksheet check BEFORE parsing
        if let Some(false) = same_worksheet(&m1.params, &m2.params) {
            return identity(m1, m2);
...
/// RemoveCol vs RemoveWorksheetMerge: shift/remove merge ranges based on removed columns
fn create_remove_col_vs_remove_merge() -> TransformFnRef {
    Arc::new(|m1: &MutationInfo, m2: &MutationInfo| {
        // Quick worksheet check BEFORE parsing
        if let Some(false) = same_worksheet(&m1.params, &m2.params) {
            return identity(m1, m2);
