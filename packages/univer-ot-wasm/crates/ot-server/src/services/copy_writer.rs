...
pub async fn copy_operations(
    ...
    // Get underlying sqlx pool from SeaORM connection
    let pool = db.get_postgres_connection_pool();

    // Use CSV format for better bytea handling (hex format \x... works in CSV)
    let mut copy_in = pool
        .copy_in_raw(
            "COPY operation_logs (doc_id, rev, user_id, mutation_id, storage_id, params, op_id, created_at) \
             FROM STDIN WITH (FORMAT csv)"
        )
        .await
        .context("Failed to start COPY")?;

    // Build CSV format data (comma separated, quoted strings)
