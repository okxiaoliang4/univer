...
use crate::transform::sheets_mutations::common::{
    shift_rule_ranges_for_insert, shift_rule_ranges_for_remove, transform_ranges_set_range_values,
};
use crate::types::{
...
#[derive(Default)]
pub struct AddRangeProtectionTransform;

impl MutationTransform for AddRangeProtectionTransform {
    ...
    fn transform_with_insert_row(
        ...
        if let Some(rules) = params.get_mut("rules").and_then(|v| v.as_array_mut()) {
    ...
    fn transform_with_remove_rows(
        ...
        if let Some(rules) = params.get_mut("rules").and_then(|v| v.as_array_mut()) {
            rules.retain_mut(|rule| {
                shift_rule_ranges_for_remove(rule, Some((remove_start, remove_end)), None)
            });
            if rules.is_empty() {
                return TransformResultInternal {
                    m1_prime: None,
    ...
    fn transform_with_remove_col(
        ...
        if let Some(rules) = params.get_mut("rules").and_then(|v| v.as_array_mut()) {
            rules.retain_mut(|rule| {
                shift_rule_ranges_for_remove(rule, None, Some((remove_start, remove_end)))
            });
            if rules.is_empty() {
                return TransformResultInternal {
                    m1_prime: None,
