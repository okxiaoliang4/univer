...
use crate::transform::sheets_mutations::common::transform_ranges_set_range_values;
use crate::transform::sheets_transform_utils::{
    shift_ranges_value_for_insert, shift_ranges_value_for_remove,
};
use crate::types::{
...
#[derive(Default)]
pub struct SetNumfmtTransform;

impl MutationTransform for SetNumfmtTransform {
    ...
    fn transform_with_insert_row(
        ...
        if let Some(values) = params.get_mut("values").and_then(|v| v.as_object_mut()) {
            for (_, value) in values.iter_mut() {
                if let Some(ranges) = value.get_mut("ranges") {
                    shift_ranges_value_for_insert(ranges, Some((insert_start, insert_count)), None);
                }
            }
        }
        TransformResultInternal {
    ...
    fn transform_with_insert_col(
        ...
        if let Some(values) = params.get_mut("values").and_then(|v| v.as_object_mut()) {
            for (_, value) in values.iter_mut() {
                if let Some(ranges) = value.get_mut("ranges") {
                    shift_ranges_value_for_insert(ranges, None, Some((insert_start, insert_count)));
                }
            }
        }
        TransformResultInternal {
    ...
    fn transform_with_remove_rows(
        ...
        if let Some(values) = params.get_mut("values").and_then(|v| v.as_object_mut()) {
            ...
            for (key, value) in values.iter_mut() {
                if let Some(ranges) = value.get_mut("ranges") {
                    if !shift_ranges_value_for_remove(
                        ranges,
