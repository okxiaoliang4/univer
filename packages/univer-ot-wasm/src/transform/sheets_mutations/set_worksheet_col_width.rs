...
#[derive(Default)]
pub struct SetWorksheetColWidthTransform;

impl MutationTransform for SetWorksheetColWidthTransform {
    ...
    fn transform_with_insert_col(
        ...
        if let Some(col_width) = params.get_mut("colWidth") {
            if let Some(col_width) = col_width.as_object_mut() {
                shift_index_map_for_insert(col_width, insert_start, insert_count);
            }
        }
        TransformResultInternal {
            m1_prime: Some(MutationInfoInternal {
