...
#[derive(Default)]
pub struct SetWorksheetRowIsAutoHeightTransform;

impl MutationTransform for SetWorksheetRowIsAutoHeightTransform {
    ...
    fn transform_with_insert_row(
        ...
        if let Some(auto_height) = params.get_mut("autoHeightInfo") {
            if let Some(auto_height) = auto_height.as_object_mut() {
                shift_index_map_for_insert(auto_height, insert_start, insert_count);
            }
        }
        TransformResultInternal {
