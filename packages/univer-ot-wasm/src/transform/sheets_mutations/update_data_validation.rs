...
#[derive(Default)]
pub struct UpdateDataValidationTransform;

impl MutationTransform for UpdateDataValidationTransform {
    ...
    fn transform_with_insert_row(
        ...
        // Check unitId and subUnitId
        let m1_unit_id = params.get("unitId").and_then(|v| v.as_str()).unwrap_or("");
        let m1_sub_unit_id = params.get("subUnitId").and_then(|v| v.as_str()).unwrap_or("");

        if m1_unit_id != m2_params.sub_unit_params.unit_id
            || m1_sub_unit_id != m2_params.sub_unit_params.sub_unit_id {
        ...
        // Handle payload with type "RANGE"
        if let Some(ref mut payload) = params.get_mut("payload") {
            if let Some(payload_obj) = payload.as_object_mut() {
                if let Some(payload_type) = payload_obj.get("type").and_then(|v| v.as_str()) {
                    if payload_type == "RANGE" {
                        if let Some(ranges) = payload_obj.get_mut("payload") {
            ...
            if let Some(payload_obj) = payload.as_object_mut() {
                if let Some(payload_type) = payload_obj.get("type").and_then(|v| v.as_str()) {
                    if payload_type == "RANGE" {
                        ...
                        // For non-RANGE types, keep the mutation
                        should_keep = true;
                    }
                } else {
                    // No type field, keep the mutation
                    should_keep = true;
                }
            }
        } else {
            // No payload, keep the mutation
            should_keep = true;
        }
